...
{
	class ProtoBuffDataRetrievalMeter : HttpDataRetrievalMeter, IDataRetrievalMeter
	{
		protected override string MediaType => "application/x-protobuf";
			...
			for(var i = minNumberOfDataPoints; i <= maxNumberOfDataPoints; i += stepSize)
			{
				...
				for(int j = 0; j < sampleSize; j++)
				{
					...
					if(responseMessage.IsSuccessStatusCode)
					{
						...
						TimeSeriesData[] timeSeries = Serializer.Deserialize<TimeSeriesData[]>(responseStream);
						stopwatch.Stop();
